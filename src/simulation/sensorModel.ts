...
export const ROTAX_912_SENSOR_ENVELOPES: Record<keyof SensorSuiteState, SensorConfig> = {
  ...
  oilPressure: { unit: 'bar', min: 0, max: 8.0, nominalRange: [3.5, 6.0], warnRange: [2.0, 7.0] },
  oilTemperature: { unit: '°C', min: 20, max: 160, nominalRange: [75, 110], warnRange: [50, 130] },
  vibration: { unit: 'mm/s', min: 0, max: 20.0, nominalRange: [0.5, 3.5], warnRange: [0, 6.0] },
...
export class SensorSuiteModel {
  ...
  public processReadings(
    ...
    (Object.keys(ROTAX_912_SENSOR_ENVELOPES) as Array<keyof SensorSuiteState>).forEach(key => {
      ...
      // Add small bounded deterministic noise if enabled and engine is running
      if (noiseEnabled && engine.engineOn && val > 0) {
      ...
      sensors[key] = {
        value: Number(val.toFixed(key === 'rpm' ? 0 : 1)),
        unit: cfg.unit,
        status,
        trend,
        min: cfg.min,
