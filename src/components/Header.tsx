...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const simStatusText = isPaused ? 'PAUSED' : engineOn ? 'RUNNING' : 'STANDBY';
  const telStatusText = isStreaming ? (isConnected ? 'STREAMING (LIVE)' : 'STREAMING (LOCAL)') : 'OFFLINE';
  ...
  return (
    <header className="bg-white border-b border-[#E5E7EB] px-4 py-2 flex flex-col gap-1.5 shadow-xs select-none">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        {/* Center: Top Required Engineering Status Badges */}
        <div className="hidden lg:flex items-center gap-2">
          {/* SIMULATION */}
          <div className="flex items-center gap-1.5 bg-[#F9FAFB] border border-[#E5E7EB] px-2.5 py-1 rounded-md text-[9px] font-mono">
            <span className={`w-2 h-2 rounded-full ${isPaused ? 'bg-amber-400 animate-pulse' : engineOn ? 'bg-[#10B981] animate-pulse' : 'bg-[#9CA3AF]'}`} />
          ...
          <div className="flex items-center gap-1.5 bg-[#F9FAFB] border border-[#E5E7EB] px-2.5 py-1 rounded-md text-[9px] font-mono">
            ...
            <span className="font-bold text-[#1F2937]">PHYSICS-BASED SIMULATION</span>
          </div>
        </div>

        {/* Right: Simulation Clock & Clock Controls */}
        <div className="flex items-center gap-2.5">
